...
if [ $? -ne 0 ]; then
  ...
  start-stop-daemon \
    ...
    -S \
    --startas /bin/bash \
    -- -c "exec kubectl -n kube-system port-forward $(kubectl -n kube-system get po -l k8s-app=kubernetes-dashboard -oname | cut -d/ -f 2) 8443:8443 &> /root/dashboard.log"
fi
...
if [ $? -ne 0 ]; then
  start-stop-daemon \
    ...
    -S \
    --startas /bin/bash \
    -- -c "exec /usr/local/bin/ngrok http --log stdout --log-level debug --host-header rewrite 8443 &> /root/ngrok.log"
  sleep 3
fi
